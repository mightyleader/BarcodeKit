...
void UPCA::encodeSymbol ( const string *data )
{
	string leftSide = data->substr( 0,6 );
	string rightSide = data->substr( 6,6 );
	...
	UPCA::encodeGuardPatterns( );
}


void UPCA::encodeQuietZones ( )
{
	xml_node< > *QZ = parsed_xml.first_node( )->first_node( )->next_sibling( )->next_sibling( )->first_node( "quietzone_left" )->first_node( "UPCA" );
	...
	QZ = parsed_xml.first_node( )->first_node( )->next_sibling( )->next_sibling( )->first_node( "quietzone_right" )->first_node( "UPCA" );
	...
	BaseBarcode::addEncodedSymbol( rqzSymbol, UPCA::getEncodedSymbols( ).size( ) );
}


void UPCA::encodeGuardPatterns( )
{
	for ( int ff = 0; ff < UPCA::getGuardPatterns( ).size( ); ff++) 
	{
		vector< int > *pattern = stringToVector( UPCA::getGuardPatterns( ).at( ff ) );
		int le, pos;
		switch ( ff ) {
			case 0:
				le = 1;
			...
			case 2:
				le = 1;
				pos = UPCA::getEncodedSymbols( ).size( );
				break;
			default:
				break;
		}
...
void UPCA::setGuardPatterns( string left, string centre, string right )
{
...
vector< string > UPCA::getGuardPatterns( )
{
	return guardPatterns;
}


vector< int >* UPCA::stringToVector( string aString )
{
	vector< int > *pattern = new vector< int >;
